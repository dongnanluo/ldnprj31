...
```bash
ldnprj31$ sam build --use-container
```
...
```bash
ldnprj31$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ldnprj31$ sam local start-api
ldnprj31$ curl http://localhost:3000/
```
...
```bash
ldnprj31$ sam logs -n HelloWorldFunction --stack-name "ldnprj31" --tail
```
...
```bash
ldnprj31$ pip install -r tests/requirements.txt --user
# unit test
ldnprj31$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
ldnprj31$ AWS_SAM_STACK_NAME="ldnprj31" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "ldnprj31"
```
